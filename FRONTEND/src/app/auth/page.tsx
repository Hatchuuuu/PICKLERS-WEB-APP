...
export default function AuthPage() {
  ...
  const strengthColors = ["bg-border-default", "bg-red-400", "bg-yellow-400", "bg-blue-400", "bg-emerald-500"];
    ...
    <div className="min-h-[100dvh] flex flex-col items-center justify-center px-4 py-4 sm:py-8 bg-background relative overflow-y-auto selection:bg-accent-primary/20">
      ...
      <div className="w-full flex justify-center scale-[0.95] sm:scale-100 origin-center">
        <motion.div layout initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ ease: [0.23, 1, 0.32, 1], duration: 0.6, layout: { type: "spring", bounce: 0, duration: 0.5 } }}
          className={cn(
            "w-full max-w-[440px] flex flex-col justify-start rounded-[24px] px-5 pt-4 pb-5 sm:px-8 sm:pt-6 sm:pb-8 relative z-10 overflow-hidden hover:overflow-y-auto bg-white/80 dark:bg-[#111f3a]/60 backdrop-blur-2xl border border-gray-200 dark:border-white/10 shadow-[0_24px_48px_rgba(0,0,0,0.1)]",
            "h-auto min-h-[440px] max-h-[85vh]"
          )}>

          <div className="text-center mb-1 sm:mb-4">
            <div className="flex items-center justify-center mb-1 mt-0">
              <PicklersLogo className="w-12 h-12 transition-transform duration-300 hover:scale-105 active:scale-95 cursor-pointer" />
            </div>
          ...
          <AnimatePresence>
            {view === "auth" && (
              <motion.div role="tablist" aria-label="Authentication modes" initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="flex relative mb-3 sm:mb-5 border-b border-solid" style={{ borderColor: "var(--border-subtle)" }}>
                {(["signin", "signup"] as const).map((val) => (
                  <button key={val} type="button" role="tab" aria-selected={tab === val} onClick={() => { form.setValue("tab", val); if (val === "signup") form.setValue("authMethod", "email"); setAuthError(""); form.clearErrors(); }} className="flex-1 py-2.5 text-[15px] font-semibold transition-colors z-10 focus-visible:outline-none focus-visible:bg-accent-primary/5 rounded-t-md"
          ...
          <form onSubmit={view === "verify-code" || view === "verify-phone" ? (e) => { e.preventDefault(); handleVerifyCode(otpCode, view === "verify-phone" ? "sms" : "recovery"); } : form.handleSubmit(onSubmit)} className="flex flex-col gap-3 sm:gap-3 flex-1 justify-start">
                ...
                <motion.div key={view} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="flex flex-col gap-2.5 sm:gap-4">
                  ...
                  {(view === "verify-code" || view === "verify-phone") && (
                    <div className="flex flex-col items-center justify-center gap-1 mt-2">
                      <OTPInput
                        ...
                        onComplete={(val) => handleVerifyCode(val, view === "verify-phone" ? "sms" : "recovery")}
                        render={({ slots }) => (
                          ...
                          {tab === "signup" && formPassword.length > 0 && (
                            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden mt-2 ml-1">
                              <div className="flex gap-1.5 h-1.5 w-full mb-1">
                                {[1, 2, 3, 4].map(level => (
                                  <div key={level} className="flex-1 rounded-full bg-border-default/50 overflow-hidden">
                              ...
                              <p className={cn("text-[10px] font-bold uppercase tracking-wider text-right transition-colors", strengthColors[strength].replace('bg-', 'text-'))}>
                  ...
                  <AnimatePresence>
                    {authError && (
                      <motion.div role="alert" aria-live="assertive" initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                        <div className="flex items-start gap-2 mt-3 mb-1 px-4 py-3 rounded-xl border shadow-[0_10px_40px_rgba(0,0,0,0.5)] backdrop-blur-2xl bg-red-500/10 border-red-500/20">
                  ...
                  <AnimatePresence>
                    {successMessage && !authError && (
                      <motion.div role="alert" aria-live="polite" initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                        <div className="flex items-start gap-2 mt-3 mb-1 px-4 py-3 rounded-xl border shadow-[0_10px_40px_rgba(0,0,0,0.5)] backdrop-blur-2xl bg-emerald-500/10 border-emerald-500/20">
                  ...
                  <motion.div layout>
                    <button type="submit" disabled={loading}
                      className={cn(
                        "w-full flex items-center justify-center gap-2 rounded-xl py-3 text-[15px] font-bold text-white shadow-lg transition-all duration-300 outline-none hover:shadow-xl mt-3 sm:mt-4 focus-visible:ring-2 focus-visible:ring-emerald-400 focus-visible:ring-offset-2",
                    ...
                    <div className="mt-4 flex flex-col items-center">
                      ...
                      <div className="flex flex-col sm:flex-row gap-3 w-full">
                        <button type="button" onClick={() => handleOAuth('google')} disabled={loading}
                          className="flex-1 flex items-center justify-center gap-2 rounded-xl py-2.5 px-4 bg-surface-interactive border border-border-default text-[14px] font-semibold text-ink-primary transition-all hover:bg-surface-hover hover:border-border-emphasis active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed">
