...
import { Check, Wallet, ScanFace, Fingerprint } from "lucide-react";
  ...
  return (
    <div className="p-4 max-w-6xl mx-auto w-full max-w-xl relative min-h-full">
        ...
        {!isAuthenticated && (
          ...
          >
            ...
            >
              ...
              <h2 className="text-[24px] font-extrabold tracking-tight text-white mb-2" style={{ fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Display'" }}>
                Secure Enclave
              </h2>
              ...
              <button 
                ...
                disabled={isScanning}
                className="w-full py-4 rounded-[20px] font-bold text-[15px] shadow-lg active:scale-[0.98] transition-all flex items-center justify-center gap-2 border border-white/10"
                style={{ 
                  background: isScanning ? "rgba(255,255,255,0.05)" : "rgba(255, 255, 255, 0.08)",
                  color: isScanning ? "var(--ink-muted)" : "var(--ink-primary)"
                }}
              >
                {isScanning ? (
                  "Scanning..."
      ...
      <section className="mb-8">
        ...
        <div className="rounded-2xl overflow-hidden shadow-sm" style={{ background: "rgba(255, 255, 255, 0.03)", border: "1px solid rgba(255, 255, 255, 0.05)" }}>
          
          <div className="flex items-center justify-between px-4 py-3" style={{ borderBottom: "1px solid rgba(255, 255, 255, 0.05)" }}>
            <span className="text-[15px] text-foreground">Logo</span>
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 rounded-full flex items-center justify-center shrink-0 overflow-hidden shadow-sm"
                style={{ background: "rgba(255,255,255,0.1)", border: "1px solid rgba(255,255,255,0.1)" }}>
                {logoPreview ? (
                  <img src={logoPreview} alt="Logo" className="w-full h-full object-cover" />
                ) : (
                  <span className="text-sm font-bold text-white">BGC</span>
                )}
              </div>
          ...
          <div className="flex items-center px-4 min-h-[44px]" style={{ borderBottom: "1px solid rgba(255, 255, 255, 0.05)" }}>
            ...
            <input value={facilityName} onChange={e => setFacilityName(e.target.value)}
              className="flex-1 bg-transparent text-[15px] text-right outline-none placeholder:text-muted-foreground"
            ...
            {!open24h && (
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }}
                ...
                <div style={{ borderTop: "1px solid rgba(255, 255, 255, 0.05)" }}>
                  <div className="flex items-center justify-between px-4 min-h-[44px]" style={{ borderBottom: "1px solid rgba(255, 255, 255, 0.05)" }}>
      ...
      <section className="mb-8">
        ...
        <div className="rounded-2xl overflow-hidden shadow-sm mb-4" style={{ background: "rgba(255, 255, 255, 0.03)", border: "1px solid rgba(255, 255, 255, 0.05)" }}>
          <div className="flex items-center justify-between px-4 min-h-[44px]">
            <div className="flex items-center gap-3">
              <div className="w-6 h-6 rounded flex items-center justify-center text-[11px] font-bold text-white shadow-sm" style={{ background: "#007DFC" }}>G</div>
          ...
          <AnimatePresence>
            {gcashEnabled && (
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }}
                transition={{ ease: "easeOut", duration: 0.2 }} className="overflow-hidden">
                <div className="flex items-center px-4 min-h-[44px]" style={{ borderTop: "1px solid rgba(255, 255, 255, 0.05)" }}>
                  ...
                  <input value={gcashNumber} onChange={e => { setGcashNumber(e.target.value); setOtpSent(false); setOtpVerified(false); }}
                    className="flex-1 bg-transparent text-[15px] text-right outline-none font-mono"
                    style={{ color: "rgba(255, 255, 255, 0.6)" }} />
                </div>
                
                {!otpVerified && (
                  <div className="flex items-center justify-between px-4 min-h-[44px]" style={{ borderTop: "1px solid rgba(255, 255, 255, 0.05)" }}>
                    {otpSent ? (
                      <>
                        <input value={otpValue} onChange={e => setOtpValue(e.target.value.slice(0, 6))} maxLength={6} placeholder="Code" className="bg-transparent text-[15px] outline-none w-20 font-mono tracking-widest" style={{ color: "rgba(255, 255, 255, 0.9)" }} />
        ...
        <div className="rounded-2xl overflow-hidden shadow-sm" style={{ background: "rgba(255, 255, 255, 0.03)", border: "1px solid rgba(255, 255, 255, 0.05)" }}>
          <div className="flex items-center justify-between px-4 min-h-[44px]">
            <div className="flex items-center gap-3">
              <div className="w-6 h-6 rounded flex items-center justify-center shadow-sm" style={{ background: "rgba(255,255,255,0.08)" }}>
      ...
      <button onClick={handleSave} disabled={saveStage === "saving"}
        className="w-full py-3.5 rounded-2xl font-bold text-sm active:scale-[0.97] flex items-center justify-center gap-2 disabled:opacity-70 mb-8"
        style={{ background: "var(--accent-success)", color: "#fff", boxShadow: "0 6px 20px rgba(34,197,94,0.25)", transition: "opacity 150ms ease-out, transform 100ms ease-out" }}
