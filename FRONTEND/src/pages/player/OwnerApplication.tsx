  ...
  return (
    <div className="p-6 max-w-2xl mx-auto pb-24">
      <button onClick={() => navigate(-1)} 
        className="group flex items-center justify-center w-11 h-11 rounded-full mb-8 transition-all active:scale-95 shadow-md"
        style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", color: "var(--ink-secondary)" }}
        onMouseEnter={e => { e.currentTarget.style.background = "rgba(255,255,255,0.1)"; e.currentTarget.style.color = "var(--ink-primary)"; }}
        onMouseLeave={e => { e.currentTarget.style.background = "rgba(255,255,255,0.05)"; e.currentTarget.style.color = "var(--ink-secondary)"; }}
        aria-label="Go Back">
        ...
        {step === 3 && (
          <div className="flex flex-col gap-5">
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="border-2 border-dashed rounded-xl p-8 text-center cursor-pointer hover:bg-white/5 transition-colors"
      ...
      <AnimatePresence>
        {showSubmitConfirm && (
          <div className="fixed inset-0 z-[100] flex items-end justify-center sm:items-center p-4 pb-8">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 bg-black/40 backdrop-blur-sm" onClick={() => setShowSubmitConfirm(false)} />
            <motion.div initial={{ y: "100%", opacity: 0.5 }} animate={{ y: 0, opacity: 1 }} exit={{ y: "100%", opacity: 0 }} transition={{ type: "spring", stiffness: 400, damping: 30 }} className="relative w-full max-w-sm flex flex-col gap-2 z-10">
              ...
              <div className="w-full max-w-sm bg-gradient-to-b from-[#1c1c1e]/95 to-[#141415]/95 backdrop-blur-[40px] rounded-[32px] overflow-hidden shadow-[0_32px_64px_-12px_rgba(0,0,0,0.6)] border border-white/[0.08]">
                 <div className="p-8 text-center pb-6">
                   ...
                   <h3 className="text-[22px] font-black text-white tracking-tight" >Submit Application?</h3>
                 ...
                 <div className="flex flex-col p-5 pt-0 gap-3">
                   ...
                   <button onClick={() => setShowSubmitConfirm(false)} className="w-full py-4 rounded-[18px] text-[16px] font-semibold text-white/80 bg-white/[0.06] hover:bg-white/[0.1] active:scale-[0.98] transition-all">
                     Review Details
                   </button>
                 </div>
              </div>
