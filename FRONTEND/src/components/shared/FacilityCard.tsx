...
export function FacilityCard({ f, onFav, onViewCourts }: { f: typeof FACILITIES[0]; onFav: () => void; onViewCourts?: () => void }) {
  ...
  return (
    <motion.div ref={ref} 
      ...
      className="rounded-[28px] overflow-hidden flex flex-col group relative cursor-pointer h-full"
      style={{ 
        background: "#121933", // Deep Navy Elevated Surface
      ...
      <div className="relative overflow-hidden h-[210px] w-full shrink-0">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-[#1c1c1e]/40 to-transparent pointer-events-none" />
        
        <span className="absolute top-4 left-4 text-[11px] px-3.5 py-1.5 rounded-[8px] font-extrabold uppercase tracking-widest text-white shadow-sm border border-white/20"
        ...
        <motion.button onClick={handleFav} aria-label={f.favorited ? "Remove favorite" : "Add to favorites"}
          ...
          <Heart className={cn("w-4 h-4 transition-colors", f.favorited ? "fill-red-500 text-red-500" : "text-white")} />
        </motion.button>
      </div>
      ...
      <div className="flex flex-col flex-1 p-5 bg-transparent">
        <div className="flex items-start justify-between gap-3 mb-1">
          <h2 className="font-semibold tracking-tight leading-tight text-[20px] text-white" >
            {f.name}
          </h2>
          <div className="flex items-center gap-1 shrink-0 mt-0.5">
            <Star className="w-4 h-4 fill-[#ff9f0a] text-[#ff9f0a]" />
            <span className="text-[14px] font-semibold text-white">{f.rating}</span>
          </div>
        </div>
        
        <div className="space-y-1.5 mt-2 mb-6">
          <div className="flex items-center gap-2 text-[14px] text-white/60 font-medium">
            <MapPin className="w-4 h-4 shrink-0" /><span>{f.location}</span>
          </div>
          <div className="flex items-center justify-between text-[14px] text-white/60 font-medium">
            <div className="flex items-center gap-2">
              <Clock className="w-4 h-4 shrink-0" /><span>{f.hours}</span>
            </div>
            <span className="text-[13px]">🏍 {f.moto} · 🚗 {f.car}</span>
          </div>
        </div>
        
        <div className="flex items-center justify-between mt-auto">
          <div className="flex flex-col">
            <span className="text-[10px] font-medium text-white/50 uppercase tracking-widest mb-0.5">Court Fee</span>
            <span className="font-semibold text-[22px] tracking-tight leading-none flex items-baseline gap-1"
                  style={{ fontFamily: "system-ui, -apple-system, BlinkMacSystemFont, 'SF Pro Display', sans-serif" }}>
              <span className="text-white">
                {minPrice === maxPrice ? `₱${minPrice}` : `₱${minPrice} - ₱${maxPrice}`}
              </span>
              <span className="font-medium text-[14px] text-white/50">/hr</span>
            </span>
          </div>
          <div
            className="text-[14px] px-5 py-2.5 rounded-[10px] font-semibold active:scale-[0.96] transition-all flex items-center justify-center"
