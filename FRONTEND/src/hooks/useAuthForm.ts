  ...
  if (data.view === "auth" || data.view === "reset-password") {
    if (data.tab === "signup" || data.view === "reset-password") {
      if (!data.password || data.password.length < 8) {
        ...
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Password must contain both letters and numbers.", path: ["password"] });
      }
...
export function useAuthForm() {
  ...
  const handleVerifyCode = async (token: string, verificationType: "recovery" | "sms" = "recovery") => {
  ...
  const handleOAuth = async (provider: 'google' | 'facebook') => {
    ...
    try {
      ...
      setLoading(false);
      shakeError(err.message || "An unexpected error occurred");
    }
  };
