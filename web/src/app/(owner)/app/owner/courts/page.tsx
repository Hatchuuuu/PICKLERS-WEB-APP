    ...
    <div className="p-4 max-w-6xl mx-auto w-full">
      <div className="relative h-[68px] mb-4 -mt-[1px] flex items-center justify-between">
        ...
        <button onClick={() => { setShowAddModal(true); setNewName(""); setNewSurface("Indoor · Hard"); setNewPrice("400"); }}
          className="flex items-center gap-1.5 px-4 py-2 mt-2 rounded-full text-sm font-bold transition-opacity duration-150 ease-out active:scale-95 shrink-0 z-10 relative shadow-[0_4px_12px_rgba(34,197,94,0.3)] bg-accent-success text-white"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {filtered.map(c => (
          <div key={c.id} className="rounded-2xl p-5 shadow-xl bg-surface-base border border-border dark:bg-white/[0.02] dark:border-white/[0.05] dark:border-t-white/10 dark:shadow-[0_8px_32px_rgba(0,0,0,0.2)] backdrop-blur-xl transition-all duration-500 ease-out" 
            style={{ 
              ...
              <>
                ...
                <div className="flex gap-3 mt-4">
                  ...
                  <button onClick={() => setManageAvailabilityId(c.id)} 
                    className="flex-1 py-3 rounded-full text-sm font-bold active:scale-[0.97] transition-all hover:opacity-90 bg-red-500/15 text-red-500 border border-red-500/30 shadow-[0_4px_12px_rgba(239,68,68,0.15)]">
                    Schedule
                  </button>
                </div>
              </>
        ...
        {showAddModal && (
          <>
            ...
            <motion.div key="add-modal" initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }}
              ...
              <div className="w-full max-w-sm rounded-2xl p-6 bg-surface-base border border-border shadow-[0_24px_64px_rgba(0,0,0,0.5)]"
                onClick={(e) => e.stopPropagation()}>
                <div className="flex items-center justify-between mb-5">
                  <h2 className="text-lg font-bold tracking-tight">List New Court</h2>
                  <button onClick={() => setShowAddModal(false)} className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-surface-interactive/80 border border-border text-muted-foreground"><X className="w-4 h-4" /></button>
                </div>
                <div className="space-y-4">
                  <div>
                    <label className="block text-sm font-medium tracking-tight mb-1.5">Court Name</label>
                    <input value={newName} onChange={e => setNewName(e.target.value)} placeholder="e.g. Court 7"
                      className="w-full px-4 py-3 rounded-xl text-sm outline-none focus:ring-2 focus:ring-ring bg-[#1A2D6E]/50 border border-border text-foreground" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium tracking-tight mb-1.5">Surface</label>
                    <select value={newSurface} onChange={e => setNewSurface(e.target.value)}
                      className="w-full px-4 py-3 rounded-xl text-sm outline-none appearance-none bg-[#1A2D6E]/50 border border-border text-foreground"
                      style={{ colorScheme: "dark" }}>
                      {surfaces.map(s => <option key={s} value={s}>{s}</option>)}
                    </select>
                  </div>
                  <div>
                    <label className="block text-sm font-medium tracking-tight mb-1.5">Price per Hour (₱)</label>
                ...
                <button onClick={addCourt} disabled={!newName.trim()}
                  className="w-full mt-5 py-3.5 rounded-2xl font-bold text-sm active:scale-[0.97] disabled:opacity-40 bg-accent-success text-white transition-opacity duration-150 ease-out">
                  Add Court
                </button>
              </div>
            </motion.div>
