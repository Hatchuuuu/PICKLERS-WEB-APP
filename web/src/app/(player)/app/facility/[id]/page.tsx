...
export default function FacilityDetailView({ facility: propFacility, onBack: propOnBack }: FacilityDetailViewProps = {}) {
  ...
  const facility = useMemo(() => {
    ...
    if (id) return facilities.find((f: Facility) => String(f.id) === String(id));
    return undefined;
  ...
  const { data: courts = [] } = useQuery({
    ...
    queryFn: async () => {
      if (!facility) return [];
      const { data, error } = await supabase.from('courts').select('*').eq('facility_id', facility.id);
        ...
        <div className="absolute bottom-6 left-0 right-0 px-6 sm:px-8">
          <div className="flex items-end justify-between gap-4 max-w-6xl mx-auto w-full">
            ...
            <div className="text-right shrink-0 pb-1 flex flex-col items-end">
              {courts.length > 0 ? (
                <>
                  <div className="text-[13px] mb-1 font-bold text-white/60 tracking-tight">from</div>
