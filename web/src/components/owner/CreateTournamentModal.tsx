...
import { X, Search, Calendar, ChevronRight, ChevronLeft, Check, ChevronDown, Trophy } from 'lucide-react';
...
export function CreateTournamentModal({ isOpen, onClose }: { isOpen: boolean, onClose: () => void }) {
    ...
    // Memoized format labels - computed once since it never changes
    ...
    const requiredPlayers = useMemo(() => {
        return playType === 'doubles' ? capacity * 2 : capacity;
    }, [playType, capacity]);

    const isRosterFull = useMemo(() => {
        return enrolledPlayers.length === requiredPlayers;
    }, [enrolledPlayers.length, requiredPlayers]);

    const isStep1Complete = useMemo(() => {
        return !!name.trim() && !!date && capacity >= 2;
    }, [name, date, capacity]);
    ...
    const isStep3Complete = useMemo(() => {
        return playType === 'singles' || (playType === 'doubles' && teams.length === capacity);
    }, [playType, teams.length, capacity]);

    // Memoized handleAutoMix function
    const handleAutoMix = useCallback(() => {
        ...
        setTeams(newTeams);
    }, [enrolledPlayers, playType, capacity]);

    // Memoized handleManualPairTap function
    const handleManualPairTap = useCallback((player: string) => {
        if (!selectedToPair) {
    ...
    }, [selectedToPair, setTeams]);

    // Memoized handleUnpair function
    const handleUnpair = useCallback((teamIndex: number) => {
        setTeams(prev => prev.filter((_, i) => i !== teamIndex));
    }, [setTeams]);

    // Memoized unassigned players calculation
    const unassignedPlayers = useMemo(() => {
        return enrolledPlayers.filter(p => !teams.some(t => t.p1 === p || t.p2 === p));
    }, [enrolledPlayers, teams]);
    ...
    }, [isStep1Complete, isStep2Complete, isStep3Complete, user, showToast, name, format, capacity, date, teams, enrolledPlayers, playType, addTournament, onClose, router]);
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    ...
                                    <div>
                                        ...
                                        <PremiumSelect
                                            value={playType}
                                            onChange={(val) => {
                                                setPlayType(val as 'singles' | 'doubles');
                                                setPairingMode('auto');
                                            }}
                                            options={['doubles', 'singles']}
                                            labelMap={{ doubles: 'Doubles', singles: 'Singles' }}
                                        />
                                    </div>
                                </div>
                                ...
                                <div>
                                    <div className="flex justify-between items-end mb-3">
                                        ...
                                        <span className={cn(
                                            ...
                                            {enrolledPlayers.length} / {requiredPlayers} Players
                                        </span>
                                    </div>
