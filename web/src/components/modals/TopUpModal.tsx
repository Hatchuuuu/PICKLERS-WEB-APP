...
export function TopUpModal() {
  const { isTopUpModalOpen, setTopUpModalOpen } = useWalletStore();
  const { user } = useAuth();
  const { createCheckoutSession, isLoading } = usePaymongo();
  ...
  const handleTopUp = () => {
    ...
    if (user?.id) {
      createCheckoutSession(amount, user.id, "Picklers Wallet Top-Up");
    }
  };
