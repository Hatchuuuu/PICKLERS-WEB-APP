...
const buttonVariants = cva(
  'cursor-pointer group whitespace-nowrap focus-visible:outline-hidden inline-flex items-center justify-center has-data-[arrow=true]:justify-between whitespace-nowrap text-sm font-medium ring-offset-background transition-[color,box-shadow,transform] duration-200 disabled:pointer-events-none disabled:opacity-60 [&_svg]:shrink-0',
  {
    variants: {
      variant: {
...
function Button({
  ...
  const BaseComp = asChild ? SlotPrimitive : 'button';
  const Comp = motion.create(BaseComp as React.ElementType);
  return (
    <Comp
      data-slot="button"
      whileTap={{ scale: 0.96 }}
